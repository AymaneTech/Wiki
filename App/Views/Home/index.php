                ...
                <div class="flex flex-col p-4 w-[380px] rounded-lg shadow-lg transition-transform hover:scale-105">
                    <div class="img w-[350px] h-[450px]">
                        <a href="<?= APP_URL ?>wikis/category/<?= $item->__get("categoryId") ?>">
                            <img src="<?= STORAGE_PATH . $item->__get("categoryImage") ?>" class="w-[100%] h-[100%] rounded-2xl shadow-lg">
                        </a>
                    </div>
                    <div class="category">
        ...
        <?php foreach ($wikis as $wiki) : ?>
            <article class="custom-card p-4 flex flex-col md:flex-row bg-white shadow-lg rounded-lg mx-4 max-w-md md:max-w-full overflow-hidden">
                <div class="md:mr-6 h-fit">
                    <img class="w-16 h-16 md:w-24 md:h-24 rounded-full object-cover shadow" src="<?= STORAGE_PATH . $wiki->author->userImage ?>" alt="avatar">
                </div>
                ...
                <div class="flex-shrink-0">
                    <img class="w-48 h-48 md:w-64 md:h-64 rounded-lg object-cover" src="<?=STORAGE_PATH . $wiki->wikiImage ?>" alt="wiki-image">
                </div>
            </article>
        <?php endforeach; ?>
    </div>
